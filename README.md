...
6. 服务发现组件使用zookeeper，缓存组件使用redis，持久化存储使用mysql并进行分表存储
...
2. 红包业务逻辑  
> 1.用户请求包红包，业务生成红包订单[分配一个全局唯一的id]，状态设置为未支付  
> 2.用户支付[本系统模拟支付成功接口]，进入发送红包流程，更新红包订单支付状态，推送红包消息  
> 3.收到红包消息的用户，查看红包消息，抢红包 ----检查是否被抢完，是否已过期，是否已经抢过
> 4.拆红包--查询是否可拆以及金额，更新红包订单，写入拆红包流水，支付转账转账[本系统忽略]  
> 5.异步化，拆红包流程中只有写红包订单之前的业务需要实时，后面的的记录流水以及金融转账等实时性不高可异步化处理  
...
> 3.红包表red_pack--红包id、用户id、金额、份数、支付状态、已抢人数  
> 4.拆红包表red_pack_detail--拆红包流水号、红包id、拆红包用户id、红包金额、时间
...
6. 数据缓存设计
> 1.用户每次发送数据会比较db内最新cid若db内较小则插入新数据，这里将用户最大cid进行缓存  
> 2.当缓存为空时或消息cid较大时，进入db流程，并重新查询db的cid后判断插入，之后删除缓存  
> 3.数据一致性问题--数据不一致只会发生在缓存cid相较于db较小，也就是缓存不能阻挡高于缓存cid低于dbcid的请求，这部分请求打入db造成的影响不大，因为单个用户的请求并发量并不大。  
> 4.接收端拉取最新数据的msg_id也按照上述逻辑进行缓存
